...
function render2(){var content=elt_ref.source.value
...
play.loop=false;evt.stopImmediatePropagation();evt.preventDefault()
...
return sym}
sym=sym.ts_prev}}
function gsot(si){var s=syms[si].p_v.sym
while(s.ts_prev)
s=s.ts_prev
return gnrn(s)}
...
play.repv=0}
play_start(si,ei)}
function edit_init(){if(typeof abc2svg!="object"||!abc2svg.modules){setTimeout(edit_init,500)
return}
...
for(var i=0;i<a.length;i++){var e=a[i];elt_ref[e]=document.getElementById(e)}
...
document.getElementById("gvl").setAttribute("value",(play.abcplay.set_vol()*10).toFixed(2))});var e=elt_ref.target;e.oncontextmenu=function(evt){var x,y,elt=evt.target,cl=elt.getAttribute('class');evt.stopImmediatePropagation();evt.preventDefault()
...
if(data.length){var reader=new FileReader();reader.onload=function(evt){elt_ref.source.value=evt.target.result;src_change()}
