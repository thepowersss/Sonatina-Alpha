...
return
var dd2,up,dx,x2,i,de3,s2=de.s,de2=de.start,s=de2.s,x=s.x+3
de.st=s2.st;de.lden=false;de.has_val=true;if(de.dd.ty=='^')
up=1
else if(de.dd.ty!='_')
...
switch(dd.glyph){case"brth":case"lphr":case"mphr":case"sphr":y=stafft+1
...
return}
if(s.nhd)
x+=s.notes[s.stem>=0?0:s.nhd].shhd;up=dd.func==4?0:dd.func==6?up6(s,de.pos):up3(s,de.pos)
if(dd.name.indexOf("invert")==0)
de.inv=1
switch(dd.ty){case'@':case'<':case'>':y=de.y
break}
if(y==undefined){if(up&&dd.name!="invertedfermata"){y=y_get(s.st,true,x-dd.wl,w)
+dd.hd
if(y<stafft)
y=stafft
if(de.y>y)
...
x+=note.shhd*stv_g.scale}else if(f_staff[dd.func]&&((de.pos&C.SL_ALI_MSK)==C.SL_CENTER||((de.pos&C.SL_ALI_MSK)==0&&!s.fmt.dynalign))&&((de.up&&st>0)||(!de.up&&st<nstaff))){if(de.up)
ym=ymid[--st]
else
...
if(i>0&&a_de[i-1].s.time==de.s.time&&a_de[i-1].dd.name.slice(0,-2)==dd.name.slice(0,-2))
de2.prev=a_de[i-1]}
...
if(!s2||s2.invis)
continue
if(user.anno_start||user.anno_stop){w=strwh(s2.text)[0]
s.wl=0;s.wr=w;s.ymn=-ht-h;s.ymx=s.ymn+h;anno_start(s)}
...
while(s2.next)
s2=s2.next}}
if(!s){draw_ties_g(s1,s2,2);s1.tie_s.ti2=s1
break}
time=s1.time+s.dur
...
if(g.sls||g.sl1)
slur++}
if(!s.p_v.ckey.k_bagpipe&&s.fmt.graceslurs&&!s.gr_shift&&!slur&&!s.tie_s&&s.next&&s.next.type==C.NOTE)
grace_slur(s)
break}}
...
if(bot<p_st.bot[i])
p_st.bot[i]=bot}}
draw_deco_staff();if(cfmt.measurenb>=0)
...
cur_sy=sy;bar_set()
continue
case C.BAR:if(s.invis||!s.bar_type)
break
...
break
case C.MREST:set_scale(s);x+=32;anno_start(s);xygl(x,staff_tb[s.st].y+12,"mrest");out_XYAB('<text style="font:bold 15px text,serif"\n\
...
return}
p=p.slice(0,n+1)}else if(p[0]=='"'){n=p.indexOf('"',1)
if(n<0){syntax(1,"No end of string in font family")
return}
p=p.slice(1,n)}else{n=p.indexOf(' ',1)
if(n>=0)
p=p.slice(0,n)}
...
if(v){f=parseFloat(v[1])
if(isNaN(f)||f<1||f>2){f=0}else if(v[2]){f2=Number(v[2])
if(isNaN(f))
...
break
case"dynalign":case"titletrim":v=Number(param)
if(isNaN(v))
...
s.ts_prev=null}
s.seqst=!s.ts_prev||s.time!=s.ts_prev.time||(w_tb[s.ts_prev.type]!=w_tb[s.type]&&w_tb[s.ts_prev.type]!=0)||s.ts_prev.v>=s.v
if(!next)
return
...
if(s==s_et)
break}}
function _bar(s){return{type:C.BAR,bar_type:"|",fname:s.fname,istart:s.istart,iend:s.iend,v:s.v,p_v:s.p_v,st:s.st,dur:0,nhd:0,notes:[{pit:s.notes?s.notes[0].pit:22}],prev:s,fmt:s.fmt}}
...
if(i!=0&&i+2==st_v.length){if(st_v[i].ymn-s.fmt.stemheight>=st_v[i+1].ymx)
...
if(g.beam_end)
break}
if(g.beam_end){if(pu+pd<mid_p*2){s.stem=1}else if(pu+pd>mid_p*2){s.stem=-1}else{if(s.fmt.bstemdown)
s.stem=-1}}}}
if(!s.stem)
...
s.k_map[(note.pit+19)%7]=note.acc}}else{s.k_map=s.k_bagpipe&&sf==0?abc2svg.keys[9]:abc2svg.keys[sf+7]}
s.k_mode=mode
...
if(s2&&s2.time==curvoice.time){if(s2&&s2.type==C.BAR){if((bar_type=="["&&!s2.text&&(curvoice.st==0||(par_sy.staves[curvoice.st-1].flags&STOP_BAR)||s.norepbra))||s2.bar_type=="|"){if(bar_type!="[")
...
s=-s}
return p0+o+(p1-p0)*s}
function do_ties(s,tie_s){var i,m,note,mid,s2
...
function sort_all(){var s,s2,p_voice,v,time,w,wmin,ir,prev,fl,new_sy,nv=voice_tb.length,vtb=[],vn=[],sy=cur_sy
for(v=0;v<nv;v++)
...
break}
continue
case C.MREST:for(s2=s.next;s2;s2=s2.next){switch(s2.type){case C.BAR:case C.GRACE:break
default:if(s2.dur)
break
continue}
break}
if(!s2||s2.type!=C.BAR){error(1,s,"Lack of bar after multi-measure rest")
s2=clone(s)
s2.type=C.BAR
s2.bar_type='|'
s2.dur=0
...
break}}
return}
function get_staves(cmd,parm){var s,p_voice,p_voice2,i,flags,v,vid,a_vf,st,range,nv=voice_tb.length,maxtime=0,no_sym=true
if(parm){a_vf=parse_staves(parm)
if(!a_vf)
return}
if(nv)
voice_adj(true);for(v=0;v<nv;v++){p_voice=voice_tb[v]
if(p_voice.time>maxtime)
maxtime=p_voice.time
if(p_voice.sym)
no_sym=false}
if(no_sym||(!maxtime&&staves_found<0)){par_sy.staves=[]
...
sym_link(s);par_sy.nstaff=nstaff;if(!parm){par_sy=s.sy=clone(par_sy,1)
...
if(vover.bar&&vover.time){sym_link({type:C.BAR,bar_type:'|',invis:1,dur:0,multi:0})}}
...
memo_kv_parm('*',a)
if(!glovar.ulen)
glovar.ulen=C.BLEN/8;case 2:goto_tune(true)
break}
if(a.length)
set_kv_parm(a)
...
if(!voice_tb[0].time){p_voice=voice_tb[0];p_voice.id=id
...
function csan_add(s){if(s.a_gch)
...
abc2svg.version="1.21.3";abc2svg.vdate="2021-09-06"
