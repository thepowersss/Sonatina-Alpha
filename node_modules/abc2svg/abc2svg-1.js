...
if(s2&&s2.time==curvoice.time){if(s2&&s2.type==C.BAR){if((bar_type=="["&&!s2.text&&(!curvoice.st||(par_sy.staves[curvoice.st-1].flags&STOP_BAR)||s.norepbra))||s2.bar_type=="|"){if(bar_type!="[")
...
function sort_all(){var s,s2,time,w,wmin,ir,fl,new_sy,nv=voice_tb.length,vtb=[],vn=[],sy=cur_sy,v=sy.top_voice,p_voice=voice_tb[v],prev={type:C.STAVES,dur:0,v:v,p_v:p_voice,time:0,st:0,sy:sy,next:p_voice.sym,seqst:true,fmt:cfmt}
...
write_text(param,'c')
return
case"clef":if(parse.state>=2){s=new_clef(param)
if(s)
get_clef(s)}
return
...
abc2svg.version="1.21.4";abc2svg.vdate="2021-10-17"
